...
\begin{verbatim}
    ...
    S = array of size (n+1, nL, nL) preserving 
        previous state (s1, s2) and assignment k
    P[0][0][0] = 0
    for i = 1 to n+1:
      roll P
      for s1 = 0 to n*L:
...
\end{enumerate}
\begin{verbatim}
    state = optimal state of n (k_n, s1_n, s2_n)
    assignment = [0]*n + [k_n]
    for i = n to 1:
      previous_state, assignment[i] = S[k_i][s1_i][s2_i]
